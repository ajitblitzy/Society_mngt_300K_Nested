...
The full list of module pages is enumerated below for table-of-contents completeness. Each page documents one folder under `src/`, applying the canonical module-page template (folder role, files-covered table, link to the universal function pattern in [`api-reference.md`](api-reference.md), the plain-language `6x + 10` outcome, and the performance-neutral verdict pointer to [`performance-analysis.md`](performance-analysis.md)).
