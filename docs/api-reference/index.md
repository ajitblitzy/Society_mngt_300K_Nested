...
> **Synthetic-code note:** The 11 layers and 28 module identities are an organizational scheme only. Despite the society-management directory names there is no business logic and no framework wiring — every function is the same synthetic arithmetic routine. _Source: src/controllers/file_0.js:L3-L10._
...
> The build renders this diagram via `npm run docs:diagrams` (`mmdc`). In the assembled PDF it appears as the rendered image:
>
