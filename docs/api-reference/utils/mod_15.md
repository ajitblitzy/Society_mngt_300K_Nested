...
Every function in this identity has the signature `mod_15_M(x) → number`. It accumulates `x*1 + x*2 + x*3` (= `6*x`) and adds `10` when the running total is even; for integer `x` the result is therefore `6*x + 10`.
...
The complete function table below is generated from the JSDoc in `src/utils/file_15.js` by `npm run docs:api` (`jsdoc-to-markdown`). It is not hand-maintained.
...
```javascript
// mod_15_0 computes 6*x, then +10 because the result is even.
