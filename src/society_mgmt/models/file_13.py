"""mod_13 - society module (models layer).

Name-preserving bindings for the original ``mod_13_0`` .. ``mod_13_1199``
functions ported from ``src/models/file_13.js``. Each binding is a thin,
...
intentionally dropped. ``models`` is a structural label only: no data-model,
schema, ORM, or persistence behavior is implemented here, because none existed
...
def _bind(name: str):
    ...
    Args:
        name: The original public function name to expose (e.g. ``mod_13_0``).

    Returns:
        A callable ``(x: int | float) -> int | float`` that returns the same
        value as the original ``mod_13_K`` function.
    """
...
# Bind every original public name (mod_13_0 .. mod_13_1199) into the module
...
# 1,200 mod_13_* callables (plus the _bind factory and the imported helper).
del _k, _fname
