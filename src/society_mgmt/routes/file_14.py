"""mod_14 - society module: name-preserving bindings for the routes layer.

Python port of the JavaScript module ``src/routes/file_14.js`` (delivered
inside ``society_mgmt_300k.zip``; there are no loose ``.js`` files in the
working tree). The 1200 byte-identical source functions ``mod_14_0`` ..
``mod_14_1199`` are regenerated here as thin, name-preserving wrappers that
...
def _make_binding(name: str) -> Callable[[int | float], int | float]:
    ...
    Args:
        name: The original public function name to expose (e.g. ``mod_14_0``).

    Returns:
        A callable ``(x: int | float) -> int | float`` that returns the same
        value as the original ``mod_14_K`` function.
    """
...
# Bind every original public name (mod_14_0 .. mod_14_1199) into the module
...
# mod_14_* callables (plus the _make_binding factory and imported helpers).
del _i, _name
